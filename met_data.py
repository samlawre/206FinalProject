...
def createtable(cur, conn):
    cur.execute('CREATE TABLE IF NOT EXISTS met (origin TEXT, date INT, credit TEXT, materials TEXT)')
...
def add_info(all_info, info_lst, cur, conn):
    for list in all_info:
        cur.execute('INSERT OR IGNORE INTO met (origin, date, credit, materials) VALUES (?, ?, ?, ?)', (info_lst.index(list[0]), list[1], list[2], str(list[3])))
    conn.commit()
...
add_info(all, unique_origin, cur, conn)
add_country(unique_origin, cur, conn)
